...
var BrowserExporter = base.createChild().addInstanceMethods({
	...
	middleware : function(){
		...
		return function(req, res, next) {
		    ...
		    if ( /^\/browserx\//.test(path) ) {
		    	...
		    	//console.log(bundles[packages[v4]]);
		    	
		    	res.send(compiler.gen_code( bundles[packages[v4]],{beautify : true} ));

		    	//next();

		    	//res.send(compiler.gen_code( sources[entryPath], {beautify : true} ));
	...
	upload : function( id ){
		...
		//bundles[id] = follow.followRequires(entryPath);
	...
	withEntryPoint : function( id ){
		...
		var bundle = follow.followRequires(entryPath);
